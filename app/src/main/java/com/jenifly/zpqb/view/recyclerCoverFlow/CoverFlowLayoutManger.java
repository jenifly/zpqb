...
public class CoverFlowLayoutManger extends RecyclerView.LayoutManager {
    ...
    /**Item间隔与item宽的比例*/
    private float mIntervalRatio = 0.858f;
...
//            cm.setSaturation(0.9f);
    ...
    private float computeScale(int x) {
        float scale = 1 - Math.abs(x - mStartX) * 0.5f / Math.abs(mStartX + mDecoratedChildWidth / mIntervalRatio);
        if (scale < 0) scale = 0;
        if (scale > 1) scale = 1;
        return scale;
    }
    ...
    /**
     * 获取中间位置
     * <p>Note:该方法主要用于{RecyclerCoverFlow#getChildDrawingOrder(int, int)}判断中间位置
