...
import com.jenifly.zpqb.view.calenderView.bizs.languages.DPLManager;
...
import com.jenifly.zpqb.view.calenderView.utils.MeasureUtil;
...
public class CalenderView extends LinearLayout {
    ...
    public DPLManager mLManager;// 语言管理器
    ...
    public CalenderView(Context context, AttributeSet attrs) {
        ...
        rlTitle.setPadding(0, rlTitlePadding, 0, rlTitlePadding);

        // 周视图根布局
        llWeek = new LinearLayout(context);
        llWeek.setBackgroundColor(mTManager.colorTitleBG());
        llWeek.setOrientation(HORIZONTAL);
      //  int llWeekPadding = MeasureUtil.dp2px(context, 10);
        llWeek.setPadding(MeasureUtil.dp2px(context, 6), MeasureUtil.dp2px(context, 3),
                MeasureUtil.dp2px(context, 6), MeasureUtil.dp2px(context, 3));
...
/*        // 标题栏子元素布局参数
     ...
     /*   tvEnsure = new TextView(context);
        ...
        for (int i = 0; i < mLManager.titleWeek().length; i++) {
            ...
            tvWeek.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 12);
       ...
       addView(llWeek, llParams);
        ...
        monthView.setMonthViewPadding(MeasureUtil.dp2px(context, 6));
        addView(monthView, llParams);
    }

    //简化设置周视图背景
    public void setWeekBackgrougColor(int color){
    ...
    public void setDecorColorT(int color) {
        monthView.setDecorColorT(color);
    }

    public void setDecorColorBG(int color) {
        monthView.setDecorColorBG(color);
    }
     ...
     */
    public void setOnDateChangListener(OnDateChangeListener onDateChangeListener) {

        monthView.setOnDateChangeListener(onDateChangeListener);
    }
