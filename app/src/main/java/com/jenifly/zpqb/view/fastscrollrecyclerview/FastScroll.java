...
public class FastScroll extends View implements RecyclerView.OnItemTouchListener {
    ...
    /*package*/ JFastScrollBar fastScrollBar;
    ...
    public FastScroll(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        fastScrollBar = new JFastScrollBar(this, attrs);
    ...
    protected boolean shouldStopScroll(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            if ((Math.abs(lastDy) < deltaThreshold && recyclerView.getScrollState() != RecyclerView.SCROLL_STATE_IDLE)) {
    ...
    public JFastScrollBar getFastScrollBar() {
    ...
    public String scrollToPositionAtProgress(float touchFraction) {
        ...
        if (!(recyclerView.getAdapter() instanceof SectionedAdapter)) {
            return "";
        }

        int posInt = (int) ((touchFraction == 1) ? itemPos - 1 : itemPos);

        SectionedAdapter sectionedAdapter = (SectionedAdapter) recyclerView.getAdapter();
    ...
    protected void getCurScrollState(ScrollPositionState stateOut) {
        ...
        stateOut.rowIndex = recyclerView.getChildPosition(child);
        if (recyclerView.getLayoutManager() instanceof GridLayoutManager) {
            stateOut.rowIndex = stateOut.rowIndex / ((GridLayoutManager) recyclerView.getLayoutManager()).getSpanCount();
        }
        stateOut.rowTopOffset = recyclerView.getLayoutManager().getDecoratedTop(child) / (float) child.getHeight();
