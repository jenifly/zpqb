package com.jenifly.zpqb.view.recyclerCoverFlow;
...
public class RecyclerCoverFlow extends RecyclerView {
    ...
    public void Nest(){
        smoothScrollToPosition(getSelectedPos()+1);
    }

    public void setCanScrollNext(int canScrollNext){
        this.canScrollNext = canScrollNext;
    }

    public int getOffsetForPosition(int position){
        return getCoverFlowLayout().calculateOffsetForPosition(position);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if(ev.getX() < mDownX && canScrollNext <= getSelectedPos()){
                    getCoverFlowLayout().setCanScrollHorizontal(false);
                }else {
                    getCoverFlowLayout().setCanScrollHorizontal(true);
                }
