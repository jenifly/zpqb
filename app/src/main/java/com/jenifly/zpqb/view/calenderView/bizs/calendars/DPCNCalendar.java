...
public class DPCNCalendar extends DPCalendar {
    ...
    private String[][] buildMonthL(int year, int month) {
        ...
        int lastmonth = 0,lastmonthtear = year,naxtmonth = 0, naxtmonthyear = year;
        if(month == 1){
            lastmonth = 12;
            lastmonthtear -= 1;
        }else
        ...
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[0].length; j++) {
                tmp[i][j] = "";
                if(i == 0 && Integer.valueOf(gregorianMonth[i][j]) > gregorianMonth[i].length){
                    tmp[i][j] =  getF(g, gregorianMonth[i][j], lastmonthtear, lastmonth);
                }else if(!TextUtils.isEmpty(gregorianMonth[i][j]) && i > 2 &&
                        Integer.valueOf(gregorianMonth[i][j]) < gregorianMonth[i].length){
                    tmp[i][j] =  getF(g, gregorianMonth[i][j], naxtmonthyear, naxtmonth);
                }else {
                    tmp[i][j] =  getF(g, gregorianMonth[i][j], year, month);
                }
            }
        }
        return tmp;
    ...
    private String getF(G g,String MonthDay,int year,int month){
        String tmp = "";
        if (!TextUtils.isEmpty(MonthDay)) {
            g.y = year;
            g.m = month;
            g.d = Integer.valueOf(MonthDay);
            L l = null;
            String result = "";
